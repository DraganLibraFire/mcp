...
class Search_Filter_Generate_Input {
	...
	*/
	...
	public function generate_range_slider($field, $min, $max, $step, $smin, $smax, $value_prefix = "", $value_postfix = "")
	{
		...
		$returnvar .= '<span class="number-value-slider sl-min">'. $value_prefix.'<input name="'.$field.'[]" type="number" min="'.esc_attr($min).'" max="'.esc_attr($max).'" step="'.esc_attr($step).'" class="range-min" value="'.(int)$smin.'" />'.$value_postfix .'</span>';
		$returnvar .= '<span class="number-crtca-slider"> - </span>';
		$returnvar .= '<span class="number-value-slider sl-max">'. $value_prefix.'<input name="'.$field.'[]" type="number" min="'.esc_attr($min).'" max="'.esc_attr($max).'" step="'.esc_attr($step).'" class="range-max" value="'.(int)$smax.'" />'.$value_postfix .'</span>';
		$returnvar .= '</div>';
