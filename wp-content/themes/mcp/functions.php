...
function get_page_by_post_name($post_name, $output = OBJECT, $post_type = 'product' ){
	global $wpdb;
	$pages = $wpdb->get_results( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type= %s", $post_name, $post_type ) );
	$return_pages = [];

	foreach ($pages as $page) {
		array_push( $return_pages, get_field('xls_index', $page->ID));
	}
